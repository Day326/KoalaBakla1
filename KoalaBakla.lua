...
elseif isOld and not isBloodMoon then
    ...
    setclipboard("https://discord.gg/vKEHurcQwA")
...
elseif isBloodMoon and not isOld then
    local e = prompt("BLOODMOON DETECTED", "Bloodmoon detected in this new server. Would you like to server-hop to find another server?")
    if e then
        nt("Server-hop accepted.", "Looking for another server...")
        local success = sh()
        if not success then
    ...
    task.wait(0.5)
    nt("Searching...", "Looking for another server...")
    local success = sh()
    if not success then
        task.wait(5)
        sh()
